...
export function Spring(options) {
	...
	// TODO: Refactor the control flow
		...
		spring.addListener({
			...
			onSpringUpdate: spr => {
				...
				if (!isColorProperty(property)) {
					...
					// Output ranges
					const t1 = Number(parseValue(output[0])[1])

					const t2 = Number(parseValue(output[1])[1])
				...
				id = window.requestAnimationFrame(() => {
					if (type === 'transform') {
            // concat the property value if transform property is already initialised
						if (!el.style['transform'].includes(property) && el.style['transform'].length !== 0) {
