...
type interpolateOptions = {
	...
	mapValues: (value: number, f1: number, f2: number, t1: number, t2: number) => void,
	// Interpolate hex colors with or without an input range
	interpolateColor: (value: number, c1: string, c2: string, f1: number, f2: number) => void,
	// Convert to degree
...
const isColorProperty = (property: property): boolean => property.includes('Color') || property.includes('color')
...
const getCallbackProps = (instance: SPRING): callbackProps => ({ ...instance.state() })
...
const createSpringWithBounciness = (bounciness: number = 20, speed: number = 10): SPRING =>
...
export function Spring(options: springOptions): SPRING {
	...
	spring.setValueVelocity = ({ value, velocity }: { value: number, velocity: number }): void =>
	...
	// Infinite iterations
	spring.infinite = (startValue: number, endValue: number, duration: number): void => {
		// Go forward
